defmodule FastSyndication.Normalization do
    ...
    def normalize_feed({:ok, :rss, data}) do
        atom_link = if data["extension"]["atom"]["link"] != nil do Enum.find(data["links"],nil,fn x -> x["rel"] == "self" end)["href"] else nil end
        {:ok, %Feed{
                feed_type: "rss",
                title: data["title"],
                description: data["description"],
                url: if atom_link != nil do atom_link else data["link"] end,
                image: data["image"],#
    ...
    def normalize_feed({:ok, :atom, data}) do
        atom_link = if data["links"] != nil do Enum.find(data["links"],nil,fn x -> x["rel"] == "self" end)["href"] else nil end
        {:ok, %Feed{
                feed_type: "atom",
    ...
    defp normalize_entry(entry,:atom) do
        ...
        entry_link = if entry["links"] != nil do Enum.find(entry["links"],nil,fn x -> x["rel"] == "self" end)["href"] else nil end
        categories = 
            if is_nil(entry["categories"]) do
                []
            else
